...
Public Class FormD3dGraphic
    ...
    Private Sub FormD3dGraphic_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Catch ex As Exception
            ...
            MsgBox("Nastala chyba pri načítaní konfigurácie," & vbCrLf & "budú obnovené základné nastavenia!" & vbCrLf & vbCrLf & "(" & ex.Message & ")", MsgBoxStyle.Exclamation, "Kontrola")
    ...
    Private Sub SimulateBlok_Old()
        ...
        PGBInitialize(iMax, "1/5 - výpočet")
        ...
        If iCount > 0 Then
            ...
            iMax = colMs.Count
            PGBInitialize(iMax, "3/5 - zlučovanie")
        ...
        Else
            MsgBox("Nenašli sa žiadne možnosti", MsgBoxStyle.Information)
        End If
        PGBFinish()
    ...
    Private Sub SimulateValec_Old()
        ...
        Clean()
        PGBInitialize(colSNKeys.Count * 3 + 3, "1/4 - výpočet")
        ...
        If iCount > 0 Then
            ...
            PGBInitialize(colMs.Count * 3 + 3, "2/4 - zlučovanie")
            For i = 1 To colMs.Count
                PGB.Value += 1
                vms = colMs(i)
                vms.GraphicsCombine()
