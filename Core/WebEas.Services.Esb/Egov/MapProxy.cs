...
namespace WebEas.Services.Esb.Egov
{
    ...
    /// Map proxy
    /// </summary>
    public class MapProxy : ProxyBase, IDisposable
    {
        private const string ServiceUrl = "https://lbsoa.intra.dcom.sk/soa/egov/map/1.0/map";
        
        private Egov.MapService.IMapServiceChannel proxy;
        
        /// <summary>
        /// Initializes a new instance of the <see cref="NasProxy" /> class.
        /// </summary>
        public MapProxy(string stsThumbprint) : base(stsThumbprint)
        {
            this.proxy = WebEas.Core.Sts.SecurityTokenServiceHelper.CreateChannelProxy<Egov.MapService.IMapServiceChannel>(ServiceUrl, stsThumbprint);
        }
        ...
        public void SaveToEvidence(long dPodanieId)
        {
            ...
            catch (FaultException<Egov.MapService.dcomFault> ex)
            {
