...
namespace WebEas.ServiceInterface
{
    public static class RepositoryExtension
    {
        ...
        {
            ModelDefinitionEgov modelDef;

            if (RepositoryExtension.typeModelDefinitionMap.TryGetValue(modelType, out modelDef))
            {
                return modelDef;
            }
        ...
        public static void SetParameterValues<T>(IDbCommand dbCmd, object obj)
        {
            ...
            foreach (IDataParameter p in dbCmd.Parameters)
            {
                ...
                if (OrmLiteConfig.DialectProvider is WebEas.Core.Base.WebEasOrmLiteDialectProvider)
                {
                    ((WebEas.Core.Base.WebEasOrmLiteDialectProvider)OrmLiteConfig.DialectProvider).SetParameterValue<T>(fieldDef, p, obj);
                }
        ...
        public static int UpdateAllData<T>(this IDbCommand dbCmd, WebEasRepositoryBase repository, IEnumerable<T> objs) where T : class, IBaseEntity
        {
            ...
            if (!objs.First().GetType().HasAttribute<TenantUpdatableAttribute>())
            {
                excludeFields.Add("D_Tenant_Id");
            }

            PrepareParameterizedUpdateStatement<T>(dialectProvider, objs.First(), dbCmd, excludeFields);
        ...
        /// <returns></returns>
        public static int DeleteData<T>(this IDbCommand dbCmd, WebEasRepositoryBase repository, object id) where T : class, IBaseEntity
        {
            T obj = null;
            try
            {
                long longId = 0;
                long.TryParse(id.ToString(), out longId);
                Type dataType = typeof(T);
                ModelDefinitionEgov modelDefinition = dataType.GetModelDefinition();
                obj = (T)Activator.CreateInstance(dataType);
                bool tenantEntityNullable = false;

                IOrmLiteDialectProvider dialectProvider = OrmLiteConfig.DialectProvider;
                obj.PrepareDeleteParameters(repository);

                if (obj is ITenantEntityNullable)
                {
                    tenantEntityNullable = ((ITenantEntityNullable)obj).D_Tenant_Id == null;
                }

                PrepareParameterizedDeleteStatement<T>(dialectProvider, dbCmd, tenantEntityNullable);
                ...
                dataType.GetProperty(primaryKeyName).SetValue(obj, id);
            ...
            {
                throw new WebEasSqlException(string.Format("Cannot delete data to {0}", typeof(T)), ex, obj != null ? obj : id);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Process the 'safe' object deletion (set DatumPlatnosti to actual date)
        /// </summary>
        /// <typeparam name="T">The type of the T.</typeparam>
        /// <param name="dbCmd">The db CMD.</param>
        /// <param name="repository">The repository.</param>
        /// <param name="id">The id.</param>
        /// <returns></returns>
        public static int SafeDeleteData<T>(this IDbCommand dbCmd, WebEasRepositoryBase repository, object id) where T : class, IBaseEntity
        {
            T obj = null;
            try
            {
                ...
                PrepareParameterizedSafeDeleteStatement<T>(dialectProvider, dbCmd, tenantEntityNullable);
            ...
            {
                throw new WebEasSqlException(string.Format("Cannot set data as deleted to {0}", typeof(T)), ex, obj != null ? obj : id);
            }
            catch (Exception ex)
            {
                throw new WebEasException(string.Format("Cannot set data as deleted to {0}", typeof(T)), ex);
            }
        }
        ...
        public static bool IsRefType(this Type fieldType)
        {
            ...
            {
                if (!JsConfig.TreatValueAsRefTypes.Contains((fieldType.IsGeneric() ? fieldType.GenericTypeDefinition() : fieldType)))
                {
                    return false;
                }
            }
        ...
        /// <returns></returns>
        public static List<T> GetList<T>(this IDbCommand dbCmd, WebEasRepositoryBase repository, Filter filter = null, PaggingParameters pagging = null, List<PfeSortAttribute> userSort = null, List<string> columnsWithData = null, Filter ribbonFilter = null) where T : class
        {
            ...
            try
            {
                ...
                PreferTenantAttribute preferTenant = isTenantEntityNullable && repository.Session.TenantId != null ? dataType.FirstAttribute<PreferTenantAttribute>() : null;
                ...
                {
                    /*if (columnsWithData == null || !columnsWithData.Contains(def.FieldName))
                    {
                        if (def.PropertyInfo.HasAttribute<PfeColumnAttribute>())
                        {
                            if (def.PropertyInfo.FirstAttribute<PfeColumnAttribute>().WithoutData)
                            {
                                continue;
                            }
                        }
                ...
                if (pagging != null && ribbonFilter == null)
                {
                ...
                sql = $"SELECT {sqlSelectPart.ToString()} FROM {sqlFromPart.ToString()}{sqlWherePart.ToString()}{sqlOrderPart.ToString()}{optionRecompile}";
                var records = GetData<T>(dbCmd, ref sql, parameters);
                if (ribbonFilter != null)
                {
                    ...
                    if (ribbonFilterSql.Contains("@"))
                    {
                        ...
                        {
                            ribbonFilterSql = Regex.Replace(ribbonFilterSql, $"@{par.Key}", $"'{par.Value.ToString()}'", RegexOptions.IgnoreCase);
                        }
                    }
                ...
                if (pagging != null)
                {
                    ...
                    {
                        //Stránkovanie na DTB
                        pagging.RecordsCount = pagging.PageSize == records.Count || pagging.PageNumber > 1 ? (int)dbCmd.Scalar($"SELECT COUNT(*) FROM {sqlFromPart.ToString()}{sqlWherePart.ToString()} ") : records.Count;
                    }
                }
        ...
        private static List<T> GetData<T>(IDbCommand dbCmd, ref string sql, Dictionary<string, object> parameters = null) where T : class
        {
            ...
            using (System.Data.IDataReader dataReader = dbCmd.ExecuteReader())
            {
                ...
                ts = new System.Collections.Generic.List<T>();
                ...
                {   
                    ...
                    t.PopulateWithSqlReader<T>(dialectProvider, dataReader, indexFieldsCache, values);
                    if (t is BaseEntity)
                    {
                        (t as BaseEntity).DefaultRecord = t.GetClone();
                    }
                    ts.Add(t);
                }
            }
        ...
        /// </summary>
        public static int GetCount<T>(this IDbCommand dbCmd, WebEasRepositoryBase repository, Filter filter = null) where T : class
        {
            string sql = string.Empty;
            try
            {
                ...
                // get records count
                sql = String.Format("SELECT COUNT(*) FROM {0}.{1} WITH(NOLOCK){2}", modelDefinition.Schema, modelDefinition.Alias, sqlWherePart.ToString());
                return repository.Db.SqlScalar<int>(sql, parameters);
            }
            catch (Exception ex)
            {
                throw new Exception(string.Format("Cannot get data from {0} - {1}", typeof(T), sql), ex);
        ...
        private static void PrepareParameterizedUpdateStatement<T>(this IOrmLiteDialectProvider dialectProvider, T obj, IDbCommand cmd, ICollection<string> excludeFields = null)
        {
            ...
            foreach (FieldDefinition fieldDefinition in modelDefinition.FieldDefinitions)
            {
                ...
                try
                {
                    if (fieldDefinition.IsPrimaryKey && (excludeFields != null || excludeFields.Count > 0))
                    {
                        if (sbWhere.Length > 0)
                        {
                            sbWhere.Append(" AND ");
            ...
            cmd.CommandText = string.Format("UPDATE {0} SET {1} {2}", dialectProvider.GetQuotedTableName(modelDefinition), sbSet, (sbWhere.Length > 0 ? string.Concat("WHERE ", sbWhere) : ""));
        }
        ...
        /// <param name="dialectProvider">The dialect provider.</param>
        /// <param name="cmd">The CMD.</param>
        private static void PrepareParameterizedDeleteStatement<T>(this IOrmLiteDialectProvider dialectProvider, IDbCommand cmd, bool tenantIdIsNull)
        {
            var stringBuilder = new StringBuilder();
            ModelDefinition modelDefinition = typeof(T).GetModelDefinition();
            cmd.Parameters.Clear();
            ...
            foreach (FieldDefinition fieldDefinition in modelDefinition.FieldDefinitions)
            {
                ...
                try
                {
                    ...
                    if (fieldDefinition.Name == "D_Tenant_Id" && (isTenantEntity || isTenantEntityNullable))
                    {
                        ...
                        {
                            stringBuilder.Append("=");
                            stringBuilder.Append(dialectProvider.GetParam(dialectProvider.SanitizeFieldNameForParamName(fieldDefinition.FieldName)));
        ...
        /// <param name="dialectProvider">The dialect provider.</param>
        /// <param name="cmd">The CMD.</param>
        private static void PrepareParameterizedSafeDeleteStatement<T>(this IOrmLiteDialectProvider dialectProvider, IDbCommand cmd, bool tenantIdIsNull)
        {
            var stringBuilder = new StringBuilder();
        ...
        private static void PrepareInsertParameters<T>(this T obj, WebEasRepositoryBase repository) where T : IBaseEntity
        {
            ...
            {
                obj.Vytvoril = ses.DcomId;
                obj.DatumVytvorenia = DateTime.Now;
                obj.Zmenil = ses.DcomId;
        ...
        static public string GetMd5fromObj<T>(this T obj)
        {
        ...
        public static DateTime GeDateFromRodneCislo(this string rc)
        {
            ...
            var validCharsLenTest = new System.Text.RegularExpressions.Regex(@"\d{9}[0-9aA]?");
        ...
        //{
        //    var nodes = Modules.FindNode(typeof(T));
