...
namespace WebEas.Esam.ServiceModel.Office.Rzp.Types
{
    ...
    [Alias("V_RzpPolZmena")]
    [DataContract]
    public class RzpPolZmenaView : RzpPolZmena, IPfeCustomize
    {
        [DataMember]
        ...
        [DataMember]
        [PfeColumn(Text = "Rozpočtová položka", RequiredFields = new[] { "Datum", "D_Program_Id" })]
        [PfeCombo(typeof(RzpPolView), NameColumn = "C_RzpPol_Id", AdditionalWhereSql = "((@D_Program_Id IS NOT NULL AND PrijemVydaj = 2) OR (@D_Program_Id IS NULL)) AND ((CASE WHEN ISDATE (@Datum) = 1 THEN @Datum END) BETWEEN PlatnostOd AND ISNULL(PlatnostDo, CASE WHEN ISDATE (@Datum) = 1 THEN @Datum END))")]
