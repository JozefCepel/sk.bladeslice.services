...
USING 
    (SELECT ID_STREDISKO,  --
     ...
     FROM OPENQUERY(URBIS, 'SELECT LTRIM(STR(@YEAR,4)) + kod_str AS ID_STREDISKO, NAZOV, CASE WHEN typ_hs = ''V'' THEN 1 ELSE 0 END AS PODCINN, PLATNOSTOD = datefromparts(@YEAR, 1, 1), CASE WHEN YEAR(GetDate()) <= @YEAR THEN Null WHEN YEAR(GetDate()) > @YEAR  THEN datefromparts(@YEAR, 12, 31) END AS DATUMPLATNOSTI, '''' AS POPIS
...
VALUES (convert(uniqueidentifier,substring(context_info(),1,16)), Source.ID_STREDISKO, Source.PORADIE, FORMAT(Source.PORADIE, '00#'), Source.NAZOV, Source.PODCINN, Source.POPIS, Source.PLATNOSTOD, Source.DATUMPLATNOSTI)
-- delete
